...
// Fetch Event - Strategy: Stale-While-Revalidate for app assets, Network-First with Cache Fallback for API/Firestore
self.addEventListener('fetch', (event) => {
  ...
  // Static Assets / App Shell Strategy (Stale-While-Revalidate)
  event.respondWith(
    caches.match(request).then((cachedResponse) => {
      const fetchPromise = fetch(request)
        ...
        .catch(() => {
          // If network fails and no cached match, return root index.html for SPA fallback
