...
async function startServer() {
  ...
  // Upload Raw Video Endpoint for Option B Cloud Video Syncing (supports JSON Base64 or Binary)
  ...
  // Fast Binary Video Upload Endpoint (Zero Base64 CPU overhead)
  app.post("/api/upload-video-binary", express.raw({ type: '*/*', limit: '100mb' }), (req, res) => {
    try {
      ...
      const rawName = req.headers['x-video-name'] as string;
  ...
  // Serverless Video Processing Endpoint (30 FPS Frame & Rule Evaluation Pipeline)
  app.post("/api/serverless-video-process", async (req, res) => {
    try {
      const { sportId, movementPhase, fps = 30, frameCount, videoDuration } = req.body;
      
      // Simulate serverless cloud execution processing at 30 FPS with skeleton & 200 rules engine
...
// Dynamic High-Precision Fallback Coaching Report Generator for when Gemini API hits quota/billing limits
function buildDynamicFallbackReport(data: any) {
  ...
  const overallGrade = score >= 9.0 ? 'Gold Star (A+)' : score >= 8.0 ? 'A Form' : 'B+ Focus';
  const summaryTitle = `${sportName} Biomechanical Precision Audit`;

  const jointFaults = jointRules.filter((r: any) => {
  ...
  const primaryAngle = measuredAngles[primaryFaultRule.id] !== undefined ? Math.round(measuredAngles[primaryFaultRule.id]) : 90;

  // Detect which sport this is to apply precise biomechanical terminology
  const normalizedSport = sportName.toLowerCase();
  const sportId = normalizedSport.includes('rugby') ? 'rugby' :
                  normalizedSport.includes('soccer') ? 'soccer' :
                  normalizedSport.includes('netball') ? 'netball' :
                  normalizedSport.includes('hockey') ? 'hockey' :
                  (normalizedSport.includes('tennis') || normalizedSport.includes('forehand')) ? 'tennis' :
                  normalizedSport.includes('golf') ? 'golf' :
                  normalizedSport.includes('cricket') ? 'cricket' : 'rugby';

  const sportSpecificInfo: Record<string, {
    headlineOptimal: string;
    headlineFault: string;
    overview: string;
    goldStandardTitle: string;
    goldStandardDesc: string;
    ...
    rugby: {
      ...
      goldStandardDesc: "Maintains a strong athletic hips hinge with a neutral back (120°-145°) and head up to prevent cervical spine axial compression entering contact.",
      goldStandardRange: "120° - 145° Spine Alignment",
      ...
      multiDrills: [
        {
          ...
          howToExecute: [
            ...
            "Hinge forward at the hips, keeping the back completely flat (130° angle).",
        ...
        },
        {
          ...
          howToExecute: [
            ...
            "Drop instantly on command into a 115° knee bend on the plant foot.",
    ...
    soccer: {
      ...
      goldStandardDesc: "Locked ankle with chest leaning forward over the ball (80°-100° trunk flexion) to suppress vertical lift and drive a highly compressed laces shot.",
      goldStandardRange: "80° - 100° Chest Flexion Over Ball",
      equipmentTools: ["Size 5 Match Ball", "Agility Ladder", "Resistance Kick Band", "Target Goal Net"],
      strengths: [
        { title: "Plant Foot Positioning", desc: "Plant foot planted parallel to the ball, absorbing landing impact safely and providing a stable foundation.", metric: "125° Optimal Plant Flexion" },
        ...
        { title: "Hip Extension Whip Speed", desc: "Backswing hip hyperextension created exceptional elastic stretch through the hip flexor complex.", metric: "165° Peak Hip Extension" }
      ],
      reps: "3 sets x 15 reps",
      ...
      multiDrills: [
        {
          ...
          howToExecute: [
            ...
            "Hinge forward at the hips until your sternum is directly over the ball (85°).",
        ...
        },
        {
          ...
          howToExecute: [
            ...
            "Immediately flex the plant knee to 125°, holding steady for 2 seconds.",
    ...
    netball: {
      ...
      goldStandardDesc: "Symmetric soft double-knee flexion (110°-130° knee angle) to safely dissipate gravity reaction forces through quadriceps and gluteals.",
      goldStandardRange: "110° - 130° Soft Knee Flexion",
      ...
      drillSteps: [
        ...
        "Immediately absorb the landing by flexing your knees to 115°, ensuring your knees point straight over your toes (avoid collapsing inward).",
      ...
      multiDrills: [
        {
          ...
          howToExecute: [
            ...
            "Land with both balls of the feet simultaneously, immediately sinking into a 120° knee bend.",
    ...
    hockey: {
      ...
      goldStandardDesc: "Deep knee crouch (110°-135° knee flexion) with low-profile stick entry angle to keep ball skimming flat across the turf.",
      goldStandardRange: "110° - 135° Crouch Knee Angle",
      ...
      drillSteps: [
        ...
        "Drop your hips low into a deep crouch, bending your lead knee to 120° while keeping your torso upright and core tight.",
      ...
      multiDrills: [
        {
          ...
          howToExecute: [
            ...
            "Drop the hips until the lead knee flexes to 115°.",
        ...
        },
        {
          ...
          howToExecute: [
            ...
            "Step into your hockey stance and rotate your shoulders 45° back without moving your hips.",
    ...
    tennis: {
      ...
      goldStandardDesc: "Hitting arm elbow flexed at a clean 90° angle (85°-110°) at shoulder level in trophy pose, preparing a fast racquet drop and high contact point.",
      goldStandardRange: "85° - 110° Elbow Trophy Position",
      ...
      strengths: [
        ...
        { title: "Deep Knee Loading Launch", desc: "Exceptional loading knee bend, translating vertical ground reaction forces into high-energy contact extension.", metric: "115° Knee Dip Loading" },
      ...
      drillSteps: [
        ...
        "Toss an imaginary ball upward while simultaneously coiling your shoulders and raising your elbow to shoulder level, flexed at 90°.",
      ...
      encouragement: "Stellar tennis analysis! Locking in that 90° elbow trophy pose is the secret to a fast, powerful serve and total protection against joint injuries.",
      multiDrills: [
        {
          name: "90° Elbow Trophy Pose Wall Alignment Holds",
          ...
          howToExecute: [
            ...
            "Raise hitting elbow to shoulder level, touching the wall with back of elbow and hand flexed at 90°.",
        ...
        },
        {
          ...
          description: "Develops 90° shoulder coiling against 45° hip angle for explosive groundstroke and serve power.",
    ...
    golf: {
      ...
      goldStandardDesc: "Maintain a steady spine tilt-angle (125°-140°) from address through impact, allowing hip rotation without vertical early extension.",
      goldStandardRange: "125° - 140° Posture Spine Hinge",
      ...
      multiDrills: [
        {
          ...
          description: "Maintains 35°-45° forward torso tilt throughout swing arc by training rear pelvic awareness.",
        ...
        },
        {
          ...
          howToExecute: [
            ...
            "Rotate shoulders 90° back, pushing the lead hand as far away from chest as comfortably possible.",
        ...
        },
        {
          ...
          reps: "3 sets x 12 reps",
          whyThisWorks: "Opens the hips 45° at impact, allowing arms to swing freely from the inside without blocking.",
    ...
    cricket: {
      ...
      goldStandardDesc: "A firmly locked, braced front leg (165°-178° extension) at plant instant, acting as a biomechanical brake to launch energy up through the torso.",
      goldStandardRange: "165° - 178° Braced Front Knee",
      equipmentTools: ["Cricket Match Ball", "Bowling Stride Marker Cones", "Target Pitch Wicket", "Resistance Brace Band"],
      strengths: [
        { title: "Bowl Arm Straightness Compliance", desc: "Kept your delivery arm beautifully straight during release, meeting strict technical regulations and optimizing leverage.", metric: "15° Legal Delivery Arm" },
      ...
      drillSteps: [
        ...
        "Immediately brace and lock your front knee straight (170° angle) while driving your trailing hip forward and over the braced leg.",
      ...
      multiDrills: [
        {
          ...
          howToExecute: [
            ...
            "Plant front heel firmly and lock the knee straight at 172°.",
        ...
        },
        {
          ...
          description: "Reinforces full arm extension from shoulder rotation, strictly complying with ICC 15° legal limits.",
  ...
  // Compute customized fault description based on actual measured angle vs rules
  let faultDesc = "";
  ...
  if (jointFaults.length > 0) {
    ...
    deviationText = `${primaryAngle}° (${isTooClosed ? 'Under-flexed / Too Closed' : 'Over-extended / Too Open'})`;
    faultImpact = isTooClosed ? "-18% Force Transfer Efficiency" : "-22% Kinetic Whip Release Speed";
    
    if (isTooClosed) {
      faultDesc = `Measured angle of ${primaryAngle}° is narrower than the biomechanical target of ${primaryFaultRule.idealMin}°-${primaryFaultRule.idealMax}°. This collapsed posture limits range of motion, restricts the muscles' eccentric load absorption, and reduces kinetic power output.`;
    } else {
      faultDesc = `Measured angle of ${primaryAngle}° is too open or over-extended compared to the biomechanical target of ${primaryFaultRule.idealMin}°-${primaryFaultRule.idealMax}°. This premature extension leaks kinetic force early and places high shear stress on joint cartilage and ligaments.`;
    }
  } else {
  ...
  // Generate dynamic areas to improve based on the exact active faults
  const customAreasToImprove = jointFaults.length > 0 ? jointFaults.slice(0, 2).map((rule: any) => {
    const angle = measuredAngles[rule.id] || 90;
    const tooClosed = angle < rule.idealMin;
    return {
      issue: `${rule.name} Deviation (${angle}°)`,
      explanation: tooClosed 
        ? `Joint angle of ${angle}° is too closed relative to the ${rule.idealMin}°-${rule.idealMax}° ideal. This causes premature load dissipation.`
        : `Joint angle of ${angle}° is too open relative to the ${rule.idealMin}°-${rule.idealMax}° ideal. This causes energy leakage and joint shear strain.`,
  ...
  return {
      ...
      takeaways: [
        {
          ...
          detail: `Your primary joint movement initiated with a solid lower-body drive, providing a ${score >= 8.0 ? 'strong' : 'functional'} momentum baseline.`
        },
        {
        ...
        },
        {
          ...
          detail: jointFaults.length === 0 
            ? 'Excellent proximal-to-distal energy transfer, translating rotation into direct terminal power.'
            : 'Slight early firing detected. Sequence adjustments will prevent energy dissipation between torso and arm release.'
        }
      ],
      weeklyPrescription: [
        {
    ...
    injuryRiskAssessment: {
      ...
      findings: kneeSafety < 80 
        ? ['Slight inward lateral knee displacement detected during peak loading frame.'] 
        : info.injuryRiskFindings,
  ...
  // Authoritative Server-Side Biomechanical & Kinetic Sequence Analysis Engine
  app.post("/api/analyze-biomechanics", async (req, res) => {
    try {
      ...
      const getBiomechanicalSequenceServer = (sportId: string) => {
        switch (sportId) {
          case 'rugby':
            return [
          ...
          default:
            return [
              ...
              { stepNumber: 3, title: '15° Bowling Arm Straightness', phaseName: 'Release Instant', keyRuleName: '15° Law Bowling Elbow' },
      ...
      const idealSequence = biomechanicalSteps.map((step: any) => step.title);
      ...
      // Calculate smoothed velocities across frames using a rolling window
      const frameVelocities = allSampledFrames.map((curr: any, i: number, arr: any[]) => {
      ...
      // Smooth velocities with a 3-frame rolling average
      const smoothedVelocities = frameVelocities.map((item: any, i: number, arr: any[]) => {
        const start = Math.max(0, i - 1);
        const end = Math.min(arr.length - 1, i + 1);
        const count = end - start + 1;

        const avgVels: Record<string, number> = {};
        jointGroups.forEach(group => {
          let sum = 0;
      ...
      const peakVelocities: { joint: string; peakTime: number; peakVelocity: number }[] = jointGroups.map(group => {
        ...
        smoothedVelocities.forEach((fv: any) => {
        ...
        // Fallback if no motion peak detected (e.g., synthetic or static video)
        if (maxVel === 0) {
          const totalDur = allSampledFrames[allSampledFrames.length - 1]?.timestamp || 3.0;
      ...
      // Strict kinetic chain check: Hips must peak before shoulders, shoulders before hands.
      // If shoulders fire before hips (> 0.03s deviation), mark sequence as incorrect and flag Step 2 (Controlled Loading / Backswing).
      ...
      const actualSteps = biomechanicalSteps.map((step: any, idx: number) => {
        ...
        // Determine exact pass/fail status per step based on strict proximal-to-distal timing
        let stepStatus: 'optimal' | 'good' | 'warning' | 'error' = 'optimal';
        let score = 92;
      ...
      const actualSequence = actualSteps.map((s: any) => s.name);
      ...
      const sequenceComparison = {
        ...
        feedback: isCorrectOrder
          ? `Exceptional kinetic sequencing! Lower body hip rotation fired before upper body shoulders and hands, maximizing rotational whip for ${sportRule.name}.`
          : `Sequence Break Detected. Upper body initiated movement before lower body hips. Ground force transfer efficiency decreased by 18%.`
      };
      ...
      const ruleResultsSummary: Record<string, any> = {};

      allSampledFrames.forEach((f: any) => {
        if (f.symmetryScore) { totalSym += f.symmetryScore; }
        if (f.kneeSafetyScore) { totalKnee += f.kneeSafetyScore; }
        if (f.angles) {
      ...
      const overallBiometricScore = Number((7.0 + (sequenceEfficiency / 40.0)).toFixed(1));
  ...
  // AI Coaching Endpoint for Unique Sports Biomechanics Analysis (Gemini 3.6 Flash)
...
- Peak Angular Velocities (°/s): ${JSON.stringify(averageVelocities || {})}
  ...
  "strengthsDetailed": [
    {
      ...
      "metric": "Measured value & status (e.g. 122° - Optimal Zone)"
    },
    {
