...
export const BiomechanicalGameArena: React.FC<Props> = ({
  ...
  // Update joints whenever sportRule or keyframeList changes
  useEffect(() => {
    const list = defaultJoints[sportId as keyof typeof defaultJoints] || defaultJoints.rugby;
    
    // Dynamically sync static template values with actual computed biometric telemetry
    const syncedList = list.map((j) => {
      let matchedAngle: number | null = null;
      
      if (keyframeList && keyframeList.length > 0) {
        // Try to find a joint rule in sportRule.jointRules that maps to this joint control
        const ruleMatches = sportRule.jointRules.filter(r => {
          const ruleIdLower = r.id.toLowerCase();
          const jIdLower = j.id.toLowerCase();
          return ruleIdLower.includes(jIdLower) || jIdLower.includes(ruleIdLower) ||
                 (j.id === 'spine' && ruleIdLower.includes('hinge')) ||
                 (j.id === 'neck' && ruleIdLower.includes('neck')) ||
                 (j.id === 'chest' && ruleIdLower.includes('chest')) ||
                 (j.id === 'ankle' && ruleIdLower.includes('ankle')) ||
                 (j.id === 'shooting_elbow' && ruleIdLower.includes('elbow')) ||
                 (j.id === 'elbow_pocket' && ruleIdLower.includes('elbow')) ||
                 (j.id === 'trophy_elbow' && ruleIdLower.includes('elbow')) ||
                 (j.id === 'wrist_snap' && ruleIdLower.includes('wrist')) ||
                 (j.id === 'lead_wrist' && ruleIdLower.includes('wrist')) ||
                 (j.id === 'front_knee' && ruleIdLower.includes('knee')) ||
                 (j.id === 'crouch_knee' && ruleIdLower.includes('knee')) ||
                 (j.id === 'knee_loading' && ruleIdLower.includes('knee')) ||
                 (j.id === 'knee_dip' && ruleIdLower.includes('knee')) ||
                 (j.id === 'knee_flex' && ruleIdLower.includes('knee')) ||
                 (j.id === 'spine_erect' && (ruleIdLower.includes('spine') || ruleIdLower.includes('torso') || ruleIdLower.includes('trunk'))) ||
                 (j.id === 'torso_hinge' && (ruleIdLower.includes('spine') || ruleIdLower.includes('torso') || ruleIdLower.includes('trunk'))) ||
                 (j.id === 'spine_angle' && (ruleIdLower.includes('spine') || ruleIdLower.includes('torso') || ruleIdLower.includes('trunk'))) ||
                 (j.id === 'trunk_flex' && (ruleIdLower.includes('spine') || ruleIdLower.includes('torso') || ruleIdLower.includes('trunk'))) ||
                 (j.id === 'lead_arm' && ruleIdLower.includes('arm')) ||
                 (j.id === 'bowl_arm' && ruleIdLower.includes('arm'));
        });
        
        if (ruleMatches.length > 0) {
          const anglesOfMatch: number[] = [];
          keyframeList.forEach(kf => {
            ruleMatches.forEach(rm => {
              if (kf.angles && kf.angles[rm.id] !== undefined && !isNaN(kf.angles[rm.id])) {
                anglesOfMatch.push(kf.angles[rm.id]);
              }
            });
          });
          
          if (anglesOfMatch.length > 0) {
            matchedAngle = Math.round(anglesOfMatch.reduce((sum, val) => sum + val, 0) / anglesOfMatch.length);
          }
        }
      }
      ...
      return {
        ...j,
        current: matchedAngle !== null ? matchedAngle : j.current
      };
    });

    setJoints(syncedList);
    setPoseMastered(false);
  }, [sportId, keyframeList, sportRule]);
