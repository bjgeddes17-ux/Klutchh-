    ...
    <div className="relative overflow-hidden bg-gradient-to-r from-amber-950/80 via-purple-950/80 to-blue-950/80 border-2 border-amber-500/50 rounded-3xl p-5 sm:p-6 shadow-[0_0_40px_rgba(245,158,11,0.25)] flex flex-col gap-4">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 z-10 relative">
        <div className="flex items-center gap-4">
          <div className="w-14 h-14 sm:w-16 sm:h-16 rounded-2xl bg-gradient-to-tr from-amber-400 via-yellow-400 to-amber-500 text-black font-black text-2xl flex items-center justify-center shadow-lg border-2 border-amber-200 shrink-0">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-4 border-t border-amber-500/20 z-10">
        {attributes.map((attr) => (
          <div key={attr.id} className="bg-zinc-950/60 border border-white/5 rounded-2xl p-3 flex flex-col gap-2">
            <span className="text-[10px] font-black text-zinc-400 uppercase tracking-tighter">{attr.label}</span>
            <div className="flex items-end gap-2">
              <span className={`text-xl sm:text-2xl font-black bg-gradient-to-br ${attr.color} bg-clip-text text-transparent font-mono`}>
                {attr.score}%
              </span>
