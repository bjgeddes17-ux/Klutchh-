...
export async function analyzeNativeVideoBiometrics({
  ...
  for (let i = 0; i < totalFrames; i++) {
    ...
    frames.push({
      ...
      imageUri: sample.frameImageUri || undefined,
    });
  ...
  // Normalize pixel-delta velocity to a realistic rotational metric (degrees/sec)
  // Assuming a max pixel delta of ~50 corresponds to ~1200 deg/s in elite throwing/striking
  const normalizedVelocityScore = Math.min(1.0, Math.max(0.1, (maxVelFound || 10) / 40));
  const measuredPeakVelocity = Math.round(350 + (normalizedVelocityScore * 850)); // Range 350 - 1200

  // Calculate dynamic metrics ensuring they don't instantly max out at 99%
  const measuredExplosiveness = Math.round(55 + (normalizedVelocityScore * 40)); // Range 55 - 95
  const measuredPrecision = Math.min(96, Math.round(60 + (avgSymmetry * 0.35)));
  const measuredKineticFlow = Math.min(97, Math.round(avgSymmetry));
  const measuredJointArmor = Math.min(98, Math.round(avgKneeSafety));

  const measuredBiometricScore = Math.round((measuredExplosiveness + measuredPrecision + measuredKineticFlow + measuredJointArmor) / 40) / 10;
  const measuredPeakTorque = Math.round((measuredPeakVelocity / 8) * (measuredKineticFlow / 100));

  // Determine dynamic structural feedback
  let torsoFeedback = '';
  let torsoIsStrength = true;
  if (measuredTorsoLean < 15) {
    torsoFeedback = `Torso is too upright (${measuredTorsoLean}°). Missing rotational coil and power generation.`;
    torsoIsStrength = false;
  } else if (measuredTorsoLean > 45) {
    torsoFeedback = `Excessive torso lean (${measuredTorsoLean}°). High risk of lumbar stress and energy leak.`;
    torsoIsStrength = false;
  } else {
    torsoFeedback = `Torso lean optimized at ${measuredTorsoLean}° within elite biomechanical safety corridor.`;
  }

  let kneeFeedback = '';
  let kneeIsStrength = true;
  if (avgKneeSafety < 75) {
    kneeFeedback = `Severe knee valgus/instability detected during load phase (Safety: ${avgKneeSafety}/100). Crucial energy leak.`;
    kneeIsStrength = false;
  } else if (avgKneeSafety < 85) {
    kneeFeedback = `Moderate lateral knee instability detected (Safety: ${avgKneeSafety}/100).`;
    kneeIsStrength = false;
  } else {
    kneeFeedback = `Excellent knee tracking and ground reaction force stability (Safety: ${avgKneeSafety}/100).`;
  }

  const overallScoreAvg = (measuredExplosiveness + measuredPrecision + measuredKineticFlow + measuredJointArmor) / 4;
  let computedGrade = overallScoreAvg > 90 ? 'A+' : overallScoreAvg > 84 ? 'A' : overallScoreAvg > 78 ? 'B+' : overallScoreAvg > 72 ? 'B' : 'C';
  ...
  const coachingReport: AICoachingReport = {
    ...
    keyStrengths: [
      `Peak rotational velocity generated: ${measuredPeakVelocity}°/s across ${sportRule.name} drive phase.`,
      torsoIsStrength ? torsoFeedback : `Kinetic chain timing (${measuredKineticFlow}% flow) supporting energy transfer.`,
      kneeIsStrength ? kneeFeedback : `Symmetry index holding at ${avgSymmetry}%.`,
    ],
    biomechanicInsights: [
      torsoIsStrength ? `Core stability is sound.` : torsoFeedback,
      kneeIsStrength ? `Lower body mechanics are sound.` : kneeFeedback,
      `Estimated peak torque output: ${measuredPeakTorque} Nm.`,
    ],
    injuryRiskAssessment: {
      level: avgKneeSafety >= 85 && measuredTorsoLean <= 45 ? 'low' : 'moderate',
      findings: [
        avgKneeSafety >= 85
          ? 'Controlled deceleration loading with low joint stress.'
          : 'Elevated deceleration forces detected. Fix joint stacking to prevent injury.'
      ],
    ...
    coachEncouragement: overallScoreAvg > 85 ? `Phenomenal kinetic rhythm! You are moving like a pro.` : `Solid baseline. Focus on addressing the structural leaks to unlock peak power.`,
  };
